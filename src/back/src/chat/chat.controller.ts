...
export class chatController {
  ...
  async getFullPartInfoNyChan(@Param('id1') chanId: number): Promise<any> {
    let someThings: any =
      await this.partService.getCompletePartecipantByChannel(chanId);
    ////console.log("QUALCOSA: ", someThings);
    return someThings;
  }
  ...
  async getChannelsInfo(
    ...
    let someThings: ChannelInfoDto[] = await this.chService.getInfoChanByUser(
      userID,
    );
    //console.log("QUALCOSA: ", someThings);
    return someThings;
  ...
  async getChannelsInfoById(
    @Param('idChan') chanId: number,
  ): Promise<ChannelInfoDto> {
    let someThings: ChannelInfoDto = await this.chService.getInfoChanById(
      chanId,
    );
    //console.log("QUALCOSA: ", someThings);
    return someThings;
  ...
  async GetPartecipantByUserAndChan(
    ...
    let someThings: Partecipant =
      await this.partService.getPartecipantByUserAndChan(userId, chanId);
    return someThings;
  }
